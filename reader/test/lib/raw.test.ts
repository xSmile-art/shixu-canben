...
describe("raw", () => {
  it("buildFileName 拼出仓库文件名", () => {
    expect(buildFileName({ num: 3, title: "雨夜" })).toBe("第003章-雨夜.md");
    expect(buildFileName({ num: 120, title: "归途" })).toBe(
      "第120章-归途.md",
    );
  });
  ...
  it("buildChapterUrl 生成编码后的 raw 地址", () => {
    ...
    expect(url).toContain(
      "https://raw.githubusercontent.com/xSmile-art/shixu-canben/main/",
    );
    expect(url).toContain("novels/%E6%97%B6%E5%BA%8F%E6%AE%8B%E6%9C%AC/");
    expect(url).toContain(encodeURIComponent("第001章-开始.md"));
  });

  it("标题含 ? # 等特殊字符时 URL 不断链", () => {
    const url = buildChapterUrl({ num: 5, title: "大结局?" });
    expect(url).not.toContain("?");
    expect(url).toContain(encodeURIComponent("第005章-大结局?.md"));
  });
