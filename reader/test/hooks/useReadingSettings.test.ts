...
describe("useReadingSettings", () => {
  ...
  it("刷新后恢复持久化设置", () => {
    localStorage.setItem(
      SETTINGS_KEY,
      JSON.stringify({ ...DEFAULT_SETTINGS, brightness: 0.6 }),
    );
    const { result } = renderHook(() => useReadingSettings());
  ...
  it("localStorage 里的非法值被 sanitize 回退默认", () => {
    localStorage.setItem(
      SETTINGS_KEY,
      JSON.stringify({ fontSize: "hello", brightness: 99, pageMode: "weird" }),
    );
    const { result } = renderHook(() => useReadingSettings());
    expect(result.current.settings.fontSize).toBe(DEFAULT_SETTINGS.fontSize);
    expect(result.current.settings.brightness).toBe(
